...
class WeekEditorFreeplayState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create() {
		...
		for (i in 0...weekFile.songs.length)
		{
			var songText:Alphabet = new Alphabet(0, (20 * i), weekFile.songs[i][0], true);
	...
	function addEditorBox() {
		...
		UI_box = new PsychUIBox(FlxG.width, FlxG.height, 250, 125, ['Freeplay']);
	...
	function addFreeplayUI() {
		...
		iconInputText = new PsychUIInputText(10, bgColorStepperR.y - 5, 100, '', 8);
