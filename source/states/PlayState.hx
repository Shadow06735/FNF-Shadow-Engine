...
class PlayState extends MusicBeatState
{
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		if(!instakillOnMiss) tempScore = Language.getPhrase('score_text', 'Score: {1} | Misses: {2} | Accuracy: {3} | Rating: {4}', [songScore, songMisses, str, finalRating]);
		else tempScore = Language.getPhrase('score_text_instakill', 'Score: {1} | Accuracy: {2} | Rating: {3}', [songScore, str, finalRating]);
	...
	override public function update(elapsed:Float)
	{
		...
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, Math.exp(-elapsed * 7.5 * camZoomingDecay * playbackRate));
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, Math.exp(-elapsed * 7.5 * camZoomingDecay * playbackRate));
		}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (daRating.ratingMod <= 0 && !note.isSustainNote)
			health -= 0.2 * healthGain;
		else if (daRating.ratingMod <= 0.34 && !note.isSustainNote)
			health -= 0.06 * healthGain;
		else if (daRating.ratingMod <= 0.67 && !note.isSustainNote)
		...
		// stealing this from Kade lmao
		var msTiming = truncateFloat(noteDiff, 3);
		
		if (currentTimingShown != null)
			comboGroup.remove(currentTimingShown);
		
		currentTimingShown = new FlxText(0, 0, 0, "0ms", 16);
		currentTimingShown.setFormat(Paths.font("pixel-latin.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		currentTimingShown.screenCenter();
		timeShown = 0;
		if (daRating.ratingMod <= 0.34)
			currentTimingShown.color = FlxColor.RED;
		else if (daRating.ratingMod <= 0.67)
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if(note != null && !note.isSustainNote) subtract = note.missHealth;
	...
	function strumPlayAnim(isDad:Bool, id:Int, time:Float) {
		...
		if(isDad) {
			...
			if(!ClientPrefs.data.opponentNote) {
				if(spr != null)
				{
					spr.playAnim('static');
					spr.resetAnim = 0;
