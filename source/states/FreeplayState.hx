...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			var songText:Alphabet = new Alphabet(0, (20 * i), songs[i].songName, true);
	...
	override function update(elapsed:Float)
	{
		...
		if (!player.playingMusic)
		{
			...
			if(songs.length > 1)
			{
				...
				{
					var checkLastHold:Int = Math.floor((holdTime - 0) * 10);
					holdTime += elapsed;
					var checkNewHold:Int = Math.floor((holdTime - 0) * 10);
		...
		// It always bugged me when mods dont do this
		// Automatically preview the song when selected
		if (instPlaying != curSelected)
		{
    		...
    		FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song), 0.01);
