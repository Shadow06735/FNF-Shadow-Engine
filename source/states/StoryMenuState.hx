...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		scoreText = new FlxText(10, 10, 0, Language.getPhrase('level_score', 'LEVEL SCORE:{1}', [lerpScore]), 36);
	...
	override function update(elapsed:Float)
	{
		...
		{
			lerpScore = Math.floor(FlxMath.lerp(intendedScore, lerpScore, 0.5));
		...
		for (num => item in grpWeekText.members)
			item.y = FlxMath.lerp(item.targetY - offY + 480, item.y, Math.exp(-elapsed * 40));
