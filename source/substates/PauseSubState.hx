...
class PauseSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		var artistTxt:FlxText = new FlxText(20, 15 + 32, 0, Language.getPhrase("artist", "Artist: {1}", [PlayState.SONG.artistName]), 32);
		...
		var blueballedTxt:FlxText = new FlxText(20, 15 + 96, 0, Language.getPhrase("blueballed", "Blueballed: {1}", [PlayState.deathCounter]), 32);
		blueballedTxt.scrollFactor.set();
		blueballedTxt.setFormat(Paths.font('vcr.ttf'), 32);
		blueballedTxt.updateHitbox();
		add(blueballedTxt);

		practiceText = new FlxText(20, 15 + 133, 0, Language.getPhrase("Practice Mode").toUpperCase(), 32);
		...
		if (controls.ACCEPT && (cantUnpause <= 0 || !controls.controllerMode))
		{
			...
			switch (daSelected)
			{
				...
				case 'Toggle Botplay':
					...
					PlayState.instance.scoreTxt.visible = (!PlayState.instance.cpuControlled && !PlayState.showOnlyStrums && !ClientPrefs.data.hideHud);
				case 'Options':
	...
	function regenMenu():Void {
		...
		for (num => str in menuItems) {
			var item = new Alphabet(0, (20 * num), Language.getPhrase('pause_$str', str), true);
